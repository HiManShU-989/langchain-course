...
async def index_documents_async(documents: List[Document], batch_size: int = 50):
    ...
    log_info(f"VectorStore Indexing: Preparing to add {len(documents)} documents to the vector store.", Colors.DARKCYAN)
    ...
    async def add_batch(batch: List[Document], batch_num: int):
        try:
            # Using aadd_documents correctly utilizes the async implementation of Pinecone 
...
async def main():
    ...
    log_info("TavilyCrawl: Starting the Crawl documentation from https://python.langchain.com/", Colors.PURPLE)
    
    # FIX: Changed `.invoke()` to `.ainvoke()` to prevent this network call from blocking the async event loop.
    ...
    # This is done to convert the results into langchain documents that can be used later while embedding and storing to pinecone.
    ...
    # Note: text_splitter.split_documents is CPU-bound and synchronous, but usually fast enough that it won't severely block the loop.
    ...
    # FIX: If you ever uncomment the line below, ensure you use single quotes inside the bracket to avoid f-string syntax errors in older Python versions (e.g., res['results']).
